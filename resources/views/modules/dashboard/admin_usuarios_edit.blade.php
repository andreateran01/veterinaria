...
<div class="d-sm-flex align-items-center justify-content-between mb-4 mt-2">
    <h1 class="h3 mb-0 text-gray-800" style="font-weight: 300;">Editar Usuario: <span class="font-weight-bold">{{ $usuario->name }}</span></h1>
</div>

<div class="card shadow-sm border-0 mb-4" style="border-left: 4px solid #343a40 !important;">
    <div class="card-header py-3 bg-white">
        <h6 class="m-0 font-weight-bold text-gray-800"><i class="fas fa-user-edit mr-1"></i> Formulario de Edición</h6>
    </div>
    <div class="card-body">
        ...
        <form action="{{ route('admin.usuarios.update', $usuario->id) }}" method="POST" enctype="multipart/form-data">
            ...
            <div class="row">
                <div class="col-md-12 mb-4">
                    ...
                    <select class="form-control" id="role" name="role" required>
                        ...
                        <option value="veterinario" {{ old('role', $usuario->role) == 'veterinario' ? 'selected' : '' }}>Veterinario</option>
                    </select>
                </div>
            </div>

            <!-- SECCIÓN DATOS DE VETERINARIO (OCULTA POR DEFECTO) -->
            <div id="veterinario_fields" style="display: {{ old('role', $usuario->role) == 'veterinario' ? 'block' : 'none' }}; background-color: #f8f9fc; padding: 20px; border-radius: 8px; border-left: 4px solid #1cc88a;" class="mb-4">
                ...
                <div class="row">
                    <div class="col-md-12 mb-3">
                        ...
                        <input type="text" class="form-control" id="nombre_completo" name="nombre_completo" value="{{ old('nombre_completo', $usuario->veterinario->nombre_completo ?? '') }}">
                ...
                <div class="row">
                    <div class="col-md-12 mb-3">
                        <label for="foto_firma">Actualizar Firma Digitalizada / Sello (Opcional)</label>
                        <input type="file" class="form-control-file mb-2" id="foto_firma" name="foto_firma" accept="image/png, image/jpeg, image/jpg">
                        <small class="text-muted d-block">Formatos admitidos: JPG, PNG. Tamaño máx: 2MB.</small>
                        @if($usuario->veterinario && $usuario->veterinario->foto_firma)
                            <div class="mt-3 p-2 bg-white rounded border d-inline-block">
                                <span class="d-block mb-1 text-sm text-gray-600">Firma actual:</span>
                                <img src="{{ asset('storage/' . $usuario->veterinario->foto_firma) }}" alt="Firma Actual" style="max-height: 80px; object-fit: contain;">
                            </div>
                        @endif
                    </div>
                </div>
            ...
            <div class="d-flex justify-content-end mt-4">
                ...
                <button type="submit" class="btn btn-primary px-4"><i class="fas fa-save mr-1"></i> Actualizar Usuario</button>
            </div>
        </form>
    </div>
</div>
